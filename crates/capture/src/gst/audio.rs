//! The per track capture branches. Each track is a set of sources mixed
//! together and encoded once:
//!
//! ```text
//! source -> queue -> audioconvert -> audioresample -> volume -> capsfilter -\
//! source -> ...                                                            -> audiomixer
//!   -> queue -> audioconvert -> capsfilter -> AAC encoder -> aacparse -> appsink
//! ```
//!
//! The source element of each device comes from the platform
//! ([`Platform::audio_source`]).
...
use openclips_core::capture::AudioTrackPlan;
...
pub fn build_track(
    ...
    for (n, source) in plan.sources.iter().enumerate() {
        ...
        let src = native.audio_source(source, &src_name)?;
...
fn sample_handler(
    ...
    move |appsink| {
        ...
        sink.on_audio(AudioPacket {
            track: index,
            pts: super::running_time(&sample, buffer.pts()),
